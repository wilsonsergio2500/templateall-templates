...
import { {Name_pascalized}FireStore } from './schema/{Name_file}.firebase';
...
import { {Name_pascalized}SetAsLoadingAction, {Name_pascalized}SetAsDoneAction, {Name_pascalized}CreateAction, {Name_pascalized}UpdateAction, {Name_pascalized}RemoveAction, {Name_pascalized}GetByIdAction, {Name_pascalized}LoadFirstPageAction, {Name_pascalized}LoadNextPageAction, {Name_pascalized}LoadPreviousPageAction } from './{Name_file}.actions';
...
export class {Name_pascalized}State {
 ...
 onDone(ctx: StateContext<I{Name_pascalized}StateModel>) {
  ...
  @Action({Name_pascalized}UpdateAction)
  onUpdateAction(ctx: StateContext<I{Name_pascalized}StateModel>, action: {Name_pascalized}UpdateAction) {
    return this.store.selectOnce(AuthState.getUser).pipe(
      mergeMap((user) => {
        const now = Date.now();
      ...
      delay(1000),
      tap(() => {
        this.snackBarStatus.OpenComplete('{Name_titlelized} Updated Succesfully');
        ctx.dispatch(new Navigate(['admin/{Name_pascalized}']));
      })
    );
  }
